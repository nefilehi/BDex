-- Active: 1724718463162@@127.0.0.1@3306
CREATE DATABASE sistema_almoxarifado;
...
SELECT fornecedor, cidade FROM tb_fornecedores;

SELECT fornecedor As Nome,cidade AS Cidade FROM tb_fornecedores
ORDER BY cidade;
...
WHERE contato="Luciana Rocha";

SELECT * FROM tb_fornecedores WHERE contato LIKE "Ma%";

SELECT descricao, qtd_atual FROM tb_materiais
WHERE qtd_atual <=10;

SELECT descricao, qtd_atual FROM tb_materiais
WHERE qtd_atual BETWEEN 10 AND 20;

SELECT MAX(qtd_atual) FROM tb_materiais;

SELECT COUNT(qtd_atual) AS EstoqueZerado
FROM tb_materiais
WHERE qtd_atual=0;

SELECT id_material,descricao FROM tb_materiais
WHERE id_segmento="1";


SELECT tb_materiais.descricao, tb_segmentos.segmento
FROM tb_materiais, tb_segmentos
WHERE tb_segmentos.id_segmento = tb_materiais.id_Segmento
AND tb_segmentos.segmento ="Limpeza";


SELECT * FROM tb_materiais WHERE qtd_atual < qtd_minima;

SELECT * FROM tb_materiais
ORDER BY qtd_atual;

SELECT * FROM tb_fornecedores WHERE cidade LIKE "M%";

SELECT * FROM tb_materiais WHERE qtd_atual BETWEEN 10 AND 50;

SELECT COUNT(id_fornecedor) FROM tb_fornecedores;

SELECT tb_materiais.descricao, tb_segmentos.segmento
FROM tb_materiais, tb_segmentos
WHERE tb_segmentos.id_segmento = tb_materiais.id_Segmento
AND tb_materiais.qtd_minima=10;
